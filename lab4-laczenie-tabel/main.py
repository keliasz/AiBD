...
def number_films_in_category(category_id:int)->pd.DataFrame:
    ...
    df = pd.read_sql(f"""
    select category.name as category, count(category.name) from category 
...
def number_film_by_length(min_length: Union[int,float] = 0, max_length: Union[int,float] = 1e6 ) :
    ...
    if min_length > max_length:
        return None
    df = pd.read_sql(f"""
        select film.length ,count(film.title) from film 
...
def client_by_sum_length(sum_min:Union[int,float])->pd.DataFrame:
    ...
    if not isinstance(sum_min, (int, float)):
        return None
    df = pd.read_sql(f"""
        select customer.first_name, customer.last_name, sum(film.length) as sum from film
        inner join inventory on inventory.film_id = film.film_id
        inner join rental on rental.inventory_id = inventory.inventory_id
...
def category_statistic_length(name:str)->pd.DataFrame:
    ...
    if not isinstance(name, str):
        return None
    df = pd.read_sql(f"""
        select category.name as category, avg(film.length), sum(film.length), min(film.length), max(film.length) from film
